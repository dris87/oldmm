...
// IP blokkolási ellenőrzés - MINDIG ELSŐKÉNT!
...
// Ha nem whitelist-en van és blokkolva van, akkor 404
if (!in_array($currentIP, $whitelistedIPs) && isIPBlocked($currentIP)) {
...
$findOffer = getOffer($con, $offer_id);
if(!isset($findOffer['status'])) {
var_dump('');
exit;
}
...
<title>Jelentkezés a <?= $findOffer['title'] ?> állásra - mumi.hu</title>
<style>
...
<div class="container-fulid">
    ...
    <div>
        <p style="text-align:center;font-size: 22px;font-weight: bold;text-transform:uppercase;"><?= $findOffer['title'] ?> (<?= $city ?>)</p>
    </div>
    <div>
        <p style="text-align:center;font-size: 16px;"><?= $findOffer['lead'] ?></p>
    </div>
    <div>
        <center><a href="https://mumi.hu/hu/allas/<?= $findOffer['slug'] ?>"><button id="" style="font-family: 'Barlow', sans-serif;text-transform: uppercase;background-color: #efefef;font-weight:bold;border-radius: 5px;padding: 10px;border:none;cursor:pointer;">Vissza a hirdetéshez</button></a>
        <br><br><br>
    </div>
    <?php if($findOffer['status'] === '5' && $findOffer['expire_date'] > date('Y-m-d')) { ?>
    <div id="info-text" class="container-fulid mt-2 border p-3 bg-light" style="background-color:#fbe5ce !important;border-color:#F79F39 !important;">
        <p style="font-size: 22px;font-weight: bold;text-transform:uppercase;">Jelentkezés</p>
        A hirdetéshez való jelentkezéshez kérjük, töltse ki az alábbi űrlapot!
        <br>
    ...
    <?php } else { ?>
        <div id="info-text" class="container-fulid mt-2 border p-3 bg-light" style="background-color:#fbe5ce !important;border-color:#F79F39 !important;text-align: center;">
        <p style="font-size: 22px;font-weight: bold;text-transform:uppercase; color: red;">Lejárt hirdetés</p>
    
        <b>Kérjük, böngésszen további állásajánlataink közözött a weboldalunkon!</b>
    </div>
    <?php } ?>
</div>
<?php if($findOffer['status'] === '5' && $findOffer['expire_date'] > date('Y-m-d')) { ?>
<div class="container-fulid mt-2 border p-3 bg-light" >
            ...
            <form class="form-horizontal" id="idForm" action="saveEmployee.php" method= "POST" enctype="multipart/form-data">
                <input type="hidden" id="offer_id" name="offer_id"  required value="<?= $findOffer['id'] ?>">
                ...
                <!-- Honeypot mező -->
                <div class="honeypot">
                    <label for="company_name">Ha látja ezt a mezőt, ne töltse ki:</label>
                ...
                <div class="form-group">
                      <label><span style="font-weight:bold;text-transform:uppercase;">Telefonszám:</span></label>
                        <input class="form-control" type="text"  id="telephone" name="telephone" value="" required>
                ...
                <div class="form-group">
                      <label><span style="font-weight:bold;text-transform:uppercase;">Megye:</span></label>
                        <select class="form-select" id="select-counties"  name="counties" placeholder="Válasszon egy megyét...">
                             <option value="">Válasszon egy megyét...</option>
                        <?php 
                ...
                <div class="form-group">
                      <label><span style="font-weight:bold;text-transform:uppercase;">Város:</span></label>
                        <select class="form-select" id="select-state" name="states" placeholder="Válasszon egy várost..." required>
                             <option value="" style="padding:10px 5px;">Válasszon egy várost...</option>
                        <?php 
                ...
                <div class="form-group">
                      <label><span style="font-weight:bold;text-transform:uppercase;">Milyen egyéb állás érdekli? </span></label>
                        <select class="form-select" id="select-categories" name="categories[]" multiple="multiple" placeholder="Válasszon egy kategóriát..." style="border:2px solid #1C75BC !important;" required>
                             <option value="" style="padding:10px 5px;">Válasszon egy kategóirát...</option>
                        <?php 
                ...
                <div class="form-group">
                      <label><span style="font-weight:bold;text-transform:uppercase;">Milyen vezetői engedélyekkel rendelkezik?</span></label>
                        <select class="form-select" id="select-driving_license" name="driving_license[]" multiple="multiple" placeholder="Válasszon egy vezetői engedélyt...">
                             <option value="" style="padding:10px 5px;">Válasszon egy vezetői...</option>
                        <?php 
                        foreach ($driving_licenses as $driving) {
                            ?>
                ...
                <div class="form-group">
                      <label><span style="font-weight:bold;text-transform:uppercase;">Önéletrajz csatolása: (pdf, jpeg, doc)</span></label>
                    <input type="file" id="cv_file" name="cv_file" value="" accept="image/png, image/jpeg, .doc, .docx, .pdf"> 
                </div>
                <br>
                ...
                <br>
                <div class="form-group">
                    <button id="submit-button" style="font-family: 'Barlow', sans-serif;text-transform: uppercase;background-color: #1C75BC;color:#ffffff;font-weight:bold;border-radius: 5px;padding: 10px;border:none;cursor:pointer;">Jelentkezem</button>
