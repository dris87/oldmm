...
<div class="row">
    <div class="col-8 d-flex">
        ...
        <div class="menu d-flex align-items-center col-9" style="top: 10px;">
            <ul class="mb-0 d-flex align-items-center">
                ...
                <?php 
                    if(in_array($_SESSION['query_user_id'], [15, 13, 11, 1, 10])) {
                ?>
                <li style="margin-left: 15px;">
                    <div class="d-flex flex-column">
                        <button id="jooble-generate" style="font-size: 12px;font-family: 'Barlow', sans-serif;text-transform: uppercase;background-color:#efefef;font-weight:bold;padding: 10px;border:none;cursor:pointer;">Jooble xml generálás</button>
                        <small class="mt-1" style="font-size: 10px; font-weight:bold;">Utoljára frissítve: <?php echo date ("Y.m.d H:i:s", filemtime('../xml/jooble.xml')); ?></small>
                    </div>
                </li>
                <li style="margin-left: 15px;">
                    <div class="d-flex flex-column">
                        <button id="indeed-generate" style="font-size: 12px;font-family: 'Barlow', sans-serif;text-transform: uppercase;background-color:#efefef;font-weight:bold;padding: 10px;border:none;cursor:pointer;">Indeed xml generálás</button>
                        <small class="mt-1" style="font-size: 10px; font-weight:bold;">Utoljára frissítve: <?php echo file_exists('../xml/indeed.xml') ? date ("Y.m.d H:i:s", filemtime('../xml/indeed.xml')) : 'Még nem generált'; ?></small>
                    </div>
                </li>
                <?php 
                    }
...
<div id="toast-alert" class="toast" role="alert" aria-live="assertive" aria-atomic="true" data-delay="3000" style="position: absolute; top: 40px; right: 20px;">
  ...
  <script>
        ...
        $("#submit-button").click(function(e){
            ...
            var table = $('#table').DataTable({
                ...
                columns: [
                    ...
                    { data: "source", title: "Forrás", width: "8%" },
                    ...
                    { data: "positions", title: "Kategóriák", width: "10%" },
   ...
   cursor_wait = function()
    ...
    $("#indeed-generate").on("click", function(e) {
        $('body').css('cursor', 'progress');
        $("#indeed-generate").prop('disabled', true);
        $.ajax({
            type: "POST",
            url: "indeed_xml.php",
            dataType:"JSON",
            success: function(data)
            {
                if(data === "ok") {
                    $('body').css('cursor', 'default');
                    $("#indeed-generate").prop('disabled', false);
                    $('#toast-alert .toast-body').html('Az Indeed XML generálás sikeres!');
                    $('#toast-alert').toast('show');
                    // Frissítsd a dátumot
                    location.reload();
                }
            ...
            error: function() {
                $('body').css('cursor', 'default');
                $("#indeed-generate").prop('disabled', false);
                alert('Hiba történt az Indeed XML generálása során!');
            }
        });
    });
