...
$("#load-offers-btn").on('click', function() {
   ...
   // Összes érintett gomb letiltása és stílus hozzáadása - hozzáadva a reset-filters gomb
   ...
   $.ajax({
       ...
       success: function(response) {
           // Gombok újra engedélyezése sikeres betöltés esetén
           $buttons.prop('disabled', false).removeClass('disabled');
           
           $('#load-offers-btn, #loading-time-warning').hide();
   
           $select.empty();
           
           // Első üres opció hozzáadása
           $select.append('<option value="" style="padding:10px 5px;">Válassz egy hirdetést...</option>');
           ...
           // Selectize inicializálás vagy frissítés
           ...
           // Az új osztályok hozzáadása
           $select.addClass('show');
           $select.closest('.form-group').find('.selectize-control').addClass('show');
           
           // Gomb konténer elrejtése
           $container.hide();
       },
       error: function(xhr, status, error) {
           // Gombok újra engedélyezése hiba esetén
           $buttons.prop('disabled', false).removeClass('disabled');
           
           $warning.addClass('d-none');
           console.error("Hiba a hirdetések betöltése során:", error);
           $btn.prop('disabled', false).removeClass('disabled').html(`
               <i class="fa fa-list" aria-hidden="true"></i> Hirdetések listájának betöltése
           `);
           alert('Hiba történt a hirdetések betöltése közben');
       }
   });
});
